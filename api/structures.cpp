...
void Message::encrypt(const secure::Key& public_key) {
  ...
  if (public_key != secure::Key::EMPTY) {
    ...
    std::string cipher_hex = cryptor.encrypt(m_message);
...
void Message::decrypt(const secure::Key& private_key) {
  ...
  if (private_key != secure::Key::EMPTY) {
    ...
    ptr = i2;              std::string cipher_hex_E = m_message.substr(ptr, E_hex_length);  // encrypted E
    ptr += E_hex_length;   std::string cipher_hex_IV = m_message.substr(ptr, IV_hex_length);  // encrypted IV
    ptr += IV_hex_length;  std::string cipher_hex_M = m_message.substr(ptr);  // encrypted message
